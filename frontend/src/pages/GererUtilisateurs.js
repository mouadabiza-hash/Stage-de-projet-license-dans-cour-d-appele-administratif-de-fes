...
function GererUtilisateurs() {
    ...
    const serviceOptions = services.map(s => ({ value: String(s.idService), label: s.nomService }));
    ...
    const handleEdit = (u) => {
        if (!perms.canManageUsers) return;
        setEditingId(u.id);
        setForm({ nomComplet: u.nomComplet, login: u.login, password: '', idService: String(u.idService), role: u.role || 'Employe' });
    };
        ...
        <div className="page-container" >
                    ...
                    <div className="form-grid">
                        ...
                        <div className="form-field">
                            ...
                            <SearchableSelect
                                name="login"
                                value={mapping.login}
                                onChange={e => setMapping({ ...mapping, login: e.target.value })}
                                options={headerOptions}
                                placeholder={`-- ${t('choisir')} --`}
                            />
                        </div>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="form-grid">
                            <div className="form-field">
                                ...
                                <input type="text" name="nomComplet" value={form.nomComplet} onChange={handleChange} required className="form-input" />
                            </div>
                            <div className="form-field">
                                <label>{t('login')} *</label>
                                <input type="text" name="login" value={form.login} onChange={handleChange} required className="form-input" />
                            </div>
                            <div className="form-field">
                            ...
                            <div className="form-field">
                                ...
                                <SearchableSelect
                                    ...
                                    options={serviceOptions}
                                    placeholder={t('selectionner_service')}
                                    required
                                />
                            </div>
                            <div className="form-field">
                                ...
                                <SearchableSelect
                                    ...
                                    onChange={handleChange}
                                    options={roleOptions}
                                    placeholder={t('select_role') || 'Sélectionner un rôle'}
                                    required
                                />
                            </div>
                        </div>
                        <div className="form-actions">
                            <button type="submit" className="btn-primary">{editingId ? t('mettre_a_jour') : t('ajouter')}</button>
            ...
            {showPasswordModal && perms.canManageUsers && (
                <div className="modal" style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: 'white', padding: '2rem', borderRadius: '1rem', zIndex: 1000, boxShadow: '0 4px 20px rgba(0,0,0,0.2)' }}>
                    ...
                    <input type="password" placeholder={t('nouveau_mot_de_passe')} value={newPassword} onChange={e => setNewPassword(e.target.value)} className="form-input" style={{ width: '100%', margin: '1rem 0' }} />
